...
void main() {
  ...
  float lightDistance = texture2D(myShadow, shadowCoordDivW.st).z;

  float shadowEffect = 1.0;

  if(myShadowCoord.w > 0.0)
    shadowEffect = lightDistance < shadowCoordDivW.z ? 0.5 : 1.0;

  gl_FragColor = shadowEffect * gl_FragColor;
}
